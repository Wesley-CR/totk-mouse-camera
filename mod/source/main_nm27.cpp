...
// stock pipeline. nm27 keeps that write and the nm24 mouse path, and drops
...
namespace {
    ...
    void LoadConfig() {
        ...
        if (R_FAILED(rc)) {
            Logging.Log("NativeMouse: nm27 config fs rc=0x%X, defaults", rc);
            return;
        }
        fsdevMountSdmc();
        std::FILE* f = std::fopen("sdmc:/NativeMouse.ini", "r");
        if (f == nullptr) {
            Logging.Log("NativeMouse: nm27 config no ini, defaults sens=1.0,1.0");
            return;
        }
        char buf[1024];
        ...
        Logging.Log("NativeMouse: nm27 config sens=%.2f,%.2f", sx, sy);
    }
    ...
    void WorkerEntry(void* arg) {
        ...
        Logging.Log("NativeMouse: nm27 worker starting service init");
        Result rc = smInitialize();
        Logging.Log("NativeMouse: nm27 worker smInitialize rc=0x%X", rc);
        if (R_FAILED(rc)) {
            return;
        }
        rc = appletInitialize();
        Logging.Log("NativeMouse: nm27 worker appletInitialize rc=0x%X", rc);
        if (R_FAILED(rc)) {
            return;
        }
        rc = hidInitialize();
        Logging.Log("NativeMouse: nm27 worker hidInitialize rc=0x%X", rc);
        if (R_FAILED(rc)) {
            return;
        }
        hidInitializeMouse();
        Logging.Log("NativeMouse: nm27 worker mouse activated");
        g_HidReady = true;
        ...
        while (true) {
            ...
            if (g_Wraps >= nextWrapLog) {
                Logging.Log("NativeMouse: nm27 worker dropped %llu wrap samples",
    ...
    void ChaseExecuteHook(void* self, void* arg) {
        ...
        if (!g_ThreadStarted) {
            ...
            Logging.Log("NativeMouse: nm27 threadCreate=0x%X threadStart=0x%X", rcT, rcS);
        }
        ...
        if (input != nullptr) {
            ...
            if (g_Calls == 1 || (g_Calls % 120) == 0) {
                Logging.Log("NativeMouse: nm27 #%llu %s wrote=%.3f,%.3f readback=%.4f,%.4f",
...
extern "C" void exl_main(void* x0, void* x1) {
    ...
    Logging.Log("NativeMouse: nm27 STEP1 entered");
    exl::hook::Initialize();
    ...
    if (hit == 0) {
        Logging.Log("NativeMouse: nm27 no signature match - idle");
        return;
    }
    ...
    Logging.Log("NativeMouse: nm27 entry=0x%llX orig=%p READY",
