...
// stock pipeline. nm25 keeps that write byte-identical and adds the mouse.
//
// Why this shape:
...
namespace {
    ...
    constexpr float kCountsToStick = 0.02f;    // mouse counts -> stick units (nm25's 0.004 felt ~5x too weak)
    ...
    void WorkerEntry(void* arg) {
        ...
        Logging.Log("NativeMouse: nm25 worker starting service init");
        Result rc = smInitialize();
        Logging.Log("NativeMouse: nm25 worker smInitialize rc=0x%X", rc);
        if (R_FAILED(rc)) {
            return;
        }
        rc = appletInitialize();
        Logging.Log("NativeMouse: nm25 worker appletInitialize rc=0x%X", rc);
        if (R_FAILED(rc)) {
            return;
        }
        rc = hidInitialize();
        Logging.Log("NativeMouse: nm25 worker hidInitialize rc=0x%X", rc);
        if (R_FAILED(rc)) {
            return;
        }
        hidInitializeMouse();
        Logging.Log("NativeMouse: nm25 worker mouse activated");
        g_HidReady = true;
    ...
    void ChaseExecuteHook(void* self, void* arg) {
        ...
        if (!g_ThreadStarted) {
            ...
            Logging.Log("NativeMouse: nm25 threadCreate=0x%X threadStart=0x%X", rcT, rcS);
        }
        ...
        if (input != nullptr) {
            ...
            if (g_Calls == 1 || (g_Calls % 120) == 0) {
                Logging.Log("NativeMouse: nm25 #%llu %s wrote=%.3f,%.3f readback=%.4f,%.4f",
...
extern "C" void exl_main(void* x0, void* x1) {
    ...
    Logging.Log("NativeMouse: nm25 STEP1 entered");
    exl::hook::Initialize();
    ...
    if (hit == 0) {
        Logging.Log("NativeMouse: nm25 no signature match - idle");
        return;
    }
    ...
    Logging.Log("NativeMouse: nm25 entry=0x%llX orig=%p READY",
