...
// stock pipeline. nm22 keeps that write byte-identical and adds the mouse.
//
// Why this shape:
...
namespace {
    ...
    void WorkerEntry(void* arg) {
        ...
        Logging.Log("NativeMouse: nm22 worker starting HID init");
        const Result rc = hidInitialize();
        Logging.Log("NativeMouse: nm22 worker hidInitialize rc=0x%X", rc);
        if (R_FAILED(rc)) {
            return;
        }
        hidInitializeMouse();
        Logging.Log("NativeMouse: nm22 worker mouse activated");
        g_HidReady = true;
    ...
    void ChaseExecuteHook(void* self, void* arg) {
        ...
        if (!g_ThreadStarted) {
            ...
            Logging.Log("NativeMouse: nm22 threadCreate=0x%X threadStart=0x%X", rcT, rcS);
        }
        ...
        if (input != nullptr) {
            ...
            if (g_Calls == 1 || (g_Calls % 120) == 0) {
                Logging.Log("NativeMouse: nm22 #%llu %s wrote=%.3f,%.3f readback=%.4f,%.4f",
...
extern "C" void exl_main(void* x0, void* x1) {
    ...
    Logging.Log("NativeMouse: nm22 STEP1 entered");
    exl::hook::Initialize();
    ...
    if (hit == 0) {
        Logging.Log("NativeMouse: nm22 no signature match - idle");
        return;
    }
    ...
    Logging.Log("NativeMouse: nm22 entry=0x%llX orig=%p READY",
