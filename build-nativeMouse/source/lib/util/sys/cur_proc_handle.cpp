...
namespace exl::util::proc_handle {
    ...
    namespace {
        ...
        Result GetViaMesosphere() {
            /* InfoType_MesosphereCurrentProcess is an Atmosphère extension, not a
             * standard svcGetInfo info type, and current libnx no longer declares
             * it (nor R_TRY).  This is only an optimisation: GetViaIpcTrick()
             * below obtains the same handle by looping a session back to
             * ourselves, and Get() already falls back to it on failure.  So when
             * the symbol is unavailable we simply report failure and let the
             * fallback run. */
#if defined(InfoType_MesosphereCurrentProcess) && defined(R_TRY)
            u64 handle;
...
#else
            return result::FailedToFindTarget;
#endif
        }
    }
